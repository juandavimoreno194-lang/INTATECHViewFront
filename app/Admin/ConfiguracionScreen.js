...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, Switch, Modal, TextInput } from "react-native";
...
import { getApiUrl, setApiUrl } from "../Regular/Herramientas/apiConfig";
...
const ConfiguracionScreen = () => {
  ...
  const nombreReal = userData?.nombre || user?.nombre || "Admin";
  ...
  return (
    ...
    <ScrollView style={{ flex: 1 }} contentContainerStyle={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.card, { backgroundColor: colors.card }]}>
        <Option icon="lock-closed-outline" text="Cambiar contraseña" onPress={() => navigation.navigate('CambiarContrasenaScreen')} colors={colors} last />
      </View>
      ...
      <Modal visible={showUrlModal} transparent animationType="fade" onRequestClose={() => setShowUrlModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.modalDesc, { color: colors.textSecondary }]}>Ingresa la dirección del servidor (ej: https://tu-api.railway.app)</Text>
            <TextInput
